...
// Create table to store profile image
db.transaction(tx => {
  tx.executeSql(
    'CREATE TABLE IF NOT EXISTS ProfileImage (id INTEGER PRIMARY KEY AUTOINCREMENT, image TEXT)',
    [],
...
// Function to save profile image to SQLite database
...
// Function to retrieve profile image from SQLite database
const getProfileImage = async () => {
  return new Promise((resolve, reject) => {
    db.transaction(tx => {
      tx.executeSql(
        'SELECT image FROM ProfileImage ORDER BY id DESC LIMIT 1',
        [],
        (_, results) => {
          if (results.rows.length > 0) {
            resolve(results.rows.item(0).image);
          } else {
            resolve(null);
          }
        },
...
function UpdateProfile() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        console.log('Error fetching image from SQLite database', error);
      }
    };
  ...
  const selectPhoto = () => {
    ImagePicker.openPicker({
      ...
      try {
        ...
        console.log('Profile image saved to SQLite database');
      } catch (error) {
        console.log('Error saving profile image to SQLite database:', error);
      }
    });
  };
  ...
  const updateProfile = () => {
    ...
    axios.post('http://13.233.116.176:3001/update-user', formdata).then(res => {
